      ...
      <div style={{ padding: `28px ${style.marginLeft ?? 48}px` }}>
        {order.map((key) => {
          switch (key) {
            ...
            case 'experience': return experience.length > 0 ? <Sec key={key} title="experience" c={c} mono={mono} sp={sp} bf={BASE_FONT}>{experience.map((e, i) => (
              <div key={e.id} style={{ marginTop: i > 0 ? '14px' : 0, paddingTop: i > 0 ? '14px' : 0, borderTop: i > 0 ? '1px dashed #e5e7eb' : 'none' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <h3 style={{ fontSize: fs(1.09), fontWeight: 700, margin: 0 }}>{e.position}</h3>
                    <p style={{ fontSize: fs(1.0), fontFamily: mono, color: c, margin: '1px 0 0' }}>{e.company}{e.location ? ` @ ${e.location}` : ''}</p>
                  </div>
                  <span style={{ fontSize: fs(0.91), fontFamily: mono, color: '#9ca3af', padding: '2px 8px', background: '#f9fafb', borderRadius: '3px', border: '1px solid #f3f4f6' }}>{formatDateRange(e.startDate, e.endDate, e.current)}</span>
                </div>
            ...
            case 'education': return education.length > 0 ? <Sec key={key} title="education" c={c} mono={mono} sp={sp} bf={BASE_FONT}>{education.map(e => (
              <div key={e.id} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                <div><h3 style={{ fontSize: fs(1.09), fontWeight: 700, margin: 0 }}>{e.degree}{e.field ? ` in ${e.field}` : ''}</h3><p style={{ fontSize: fs(0.95), color: '#6b7280', margin: 0 }}>{e.institution}{e.gpa ? ` · GPA: ${e.gpa}` : ''}</p></div>
                <span style={{ fontSize: fs(0.91), fontFamily: mono, color: '#9ca3af' }}>{formatDateRange(e.startDate, e.endDate, false)}</span>
              </div>
            ))}</Sec> : null;
            case 'skills': return skills.length > 0 ? <Sec key={key} title="tech_stack" c={c} mono={mono} sp={sp} bf={BASE_FONT}><div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>{skills.flatMap(s => s.items.map((item, j) => (
            ...
            case 'projects': return projects.length > 0 ? <Sec key={key} title="projects" c={c} mono={mono} sp={sp} bf={BASE_FONT}>{projects.map((p, i) => (
            ...
            case 'certifications': return certifications.length > 0 ? <Sec key={key} title="certifications" c={c} mono={mono} sp={sp} bf={BASE_FONT}>{certifications.map(cert => <p key={cert.id} style={{ fontSize: fs(0.95), margin: '3px 0', fontFamily: mono }}><span style={{ color: c }}>→</span> {cert.name} <span style={{ color: '#aaa' }}>({cert.issuer}, {formatDate(cert.date)})</span></p>)}</Sec> : null;
            case 'languages': return languages.length > 0 ? <Sec key={key} title="languages" c={c} mono={mono} sp={sp} bf={BASE_FONT}><div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>{languages.map(l => <span key={l.id} style={{ fontSize: fs(0.95), fontFamily: mono }}>{l.language}: <span style={{ color: '#9ca3af' }}>{l.proficiency}</span></span>)}</div></Sec> : null;
            case 'awards': return awards.length > 0 ? <Sec key={key} title="awards" c={c} mono={mono} sp={sp} bf={BASE_FONT}>{awards.map(a => <p key={a.id} style={{ fontSize: fs(0.95), margin: '3px 0' }}>★ {a.title}{a.issuer ? ` — ${a.issuer}` : ''}</p>)}</Sec> : null;
            case 'volunteering': return volunteering.length > 0 ? <Sec key={key} title="volunteering" c={c} mono={mono} sp={sp} bf={BASE_FONT}>{volunteering.map(v => <p key={v.id} style={{ fontSize: fs(0.95), margin: '3px 0' }}>{v.role} @ {v.organization} ({formatDateRange(v.startDate, v.endDate, v.current)})</p>)}</Sec> : null;
            case 'publications': return publications.length > 0 ? <Sec key={key} title="publications" c={c} mono={mono} sp={sp} bf={BASE_FONT}>{publications.map(p => <p key={p.id} style={{ fontSize: fs(0.95), fontStyle: 'italic', margin: '3px 0' }}>{p.title} — {p.publisher}, {formatDate(p.date)}</p>)}</Sec> : null;
            case 'references': return references.length > 0 ? <Sec key={key} title="references" c={c} mono={mono} sp={sp} bf={BASE_FONT}>{references.map(r => <div key={r.id} style={{ marginBottom: '6px' }}><p style={{ fontSize: fs(0.95), fontWeight: 600, margin: 0 }}>{r.name}</p><p style={{ fontSize: fs(0.86), color: '#999', margin: 0 }}>{r.position}, {r.company}</p></div>)}</Sec> : null;
            default: return null;
          }
        })}
      </div>
