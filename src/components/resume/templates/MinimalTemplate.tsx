  ...
  return (
    <div style={{ width: '794px', minHeight: '1123px', padding: `${style.marginTop ?? 56}px ${style.marginRight ?? 72}px ${style.marginBottom ?? 56}px ${style.marginLeft ?? 72}px`, background: '#fff', fontFamily: style.fontFamily, fontSize: `${BASE_FONT}px`, lineHeight: style.lineHeight ?? 1.6, color: '#2d2d2d' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '10px' }}>
        <h1 style={{ fontSize: fs(2.55), fontWeight: 300, letterSpacing: '4px', margin: 0, textTransform: 'uppercase', color: '#111' }}>{pi.firstName} {pi.lastName}</h1>
        ...
        switch (key) {
          ...
          case 'experience': return experience.length > 0 ? <Sec key={key} title="Experience" sp={sp} bf={BASE_FONT}>{experience.map((e, i) => (
            <div key={e.id} style={{ marginTop: i > 0 ? '16px' : 0 }}>
              ...
              {e.highlights.filter(Boolean).length > 0 && <ul style={{ margin: '4px 0 0 16px', padding: 0 }}>{e.highlights.filter(Boolean).map((h, j) => (
                <li key={j} style={{ fontSize: fs(0.95), color: '#555', marginTop: `${psp}px` }}>{h}</li>
              ))}</ul>}
            </div>
          ))}</Sec> : null;
          ...
          case 'projects': return projects.length > 0 ? <Sec key={key} title="Projects" sp={sp} bf={BASE_FONT}>{projects.map((p, i) => (
            <div key={p.id} style={{ marginTop: i > 0 ? '10px' : 0 }}>
              <span style={{ fontWeight: 600, fontSize: fs(1.05) }}>{p.name}</span>{p.url && <span style={{ fontSize: fs(0.86), color: '#999', marginLeft: '8px' }}>{p.url}</span>}
