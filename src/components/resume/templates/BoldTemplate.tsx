      ...
      <div style={{ padding: `24px ${style.marginRight ?? 48}px 24px ${style.marginLeft ?? 48}px` }}>
        {order.map((key) => {
          switch (key) {
            ...
            case 'experience': return experience.length > 0 ? <Sec key={key} title="WORK EXPERIENCE" c={c} sp={sp} bf={BASE_FONT}>{experience.map((e, i) => (
              <div key={e.id} style={{ marginTop: i > 0 ? '16px' : 0, paddingTop: i > 0 ? '16px' : 0, borderTop: i > 0 ? '2px solid #f3f4f6' : 'none' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <h3 style={{ fontSize: fs(1.27), fontWeight: 800, color: '#111', margin: 0 }}>{e.position}</h3>
                  ...
                  <span style={{ fontSize: fs(0.91), fontWeight: 700, color: '#fff', padding: '4px 12px', background: c, borderRadius: '4px', flexShrink: 0 }}>{formatDateRange(e.startDate, e.endDate, e.current)}</span>
                </div>
            ...
            case 'education': return education.length > 0 ? <Sec key={key} title="EDUCATION" c={c} sp={sp} bf={BASE_FONT}>{education.map(e => (
              <div key={e.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                <div>
                  <h3 style={{ fontSize: fs(1.18), fontWeight: 800, margin: 0 }}>{e.degree}{e.field ? ` in ${e.field}` : ''}</h3>
                ...
                <span style={{ fontSize: fs(0.91), fontWeight: 700, color: '#fff', padding: '4px 12px', background: c, borderRadius: '4px', flexShrink: 0 }}>{formatDateRange(e.startDate, e.endDate, false)}</span>
              </div>
            ))}</Sec> : null;
            case 'skills': return skills.length > 0 ? <Sec key={key} title="SKILLS" c={c} sp={sp} bf={BASE_FONT}>{skills.map(s => (
            ...
            case 'projects': return projects.length > 0 ? <Sec key={key} title="PROJECTS" c={c} sp={sp} bf={BASE_FONT}>{projects.map((p, i) => (
            ...
            case 'certifications': return certifications.length > 0 ? <Sec key={key} title="CERTIFICATIONS" c={c} sp={sp} bf={BASE_FONT}>{certifications.map(cert => (
            ...
            case 'languages': return languages.length > 0 ? <Sec key={key} title="LANGUAGES" c={c} sp={sp} bf={BASE_FONT}><div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>{languages.map(l => (
            ...
            case 'awards': return awards.length > 0 ? <Sec key={key} title="AWARDS" c={c} sp={sp} bf={BASE_FONT}>{awards.map(a => <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}><div style={{ width: '6px', height: '6px', background: c, borderRadius: '1px' }} /><span style={{ fontSize: fs(0.95) }}><strong>{a.title}</strong>{a.issuer ? ` — ${a.issuer}` : ''}</span></div>)}</Sec> : null;
            case 'volunteering': return volunteering.length > 0 ? <Sec key={key} title="VOLUNTEERING" c={c} sp={sp} bf={BASE_FONT}>{volunteering.map(v => <div key={v.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: fs(0.95), margin: '4px 0' }}><span><strong>{v.role}</strong> — {v.organization}</span><span style={{ color: '#9ca3af', fontWeight: 600 }}>{formatDateRange(v.startDate, v.endDate, v.current)}</span></div>)}</Sec> : null;
            case 'publications': return publications.length > 0 ? <Sec key={key} title="PUBLICATIONS" c={c} sp={sp} bf={BASE_FONT}>{publications.map(p => <p key={p.id} style={{ fontSize: fs(0.95), margin: '3px 0' }}><strong>{p.title}</strong> — {p.publisher}, {formatDate(p.date)}</p>)}</Sec> : null;
            case 'references': return references.length > 0 ? <Sec key={key} title="REFERENCES" c={c} sp={sp} bf={BASE_FONT}>{references.map(r => <div key={r.id} style={{ marginBottom: '6px' }}><p style={{ fontSize: fs(1.0), fontWeight: 800, margin: 0 }}>{r.name}</p><p style={{ fontSize: fs(0.91), color: '#6b7280', margin: 0 }}>{r.position}, {r.company}</p></div>)}</Sec> : null;
            default: return null;
          }
        })}
      </div>
